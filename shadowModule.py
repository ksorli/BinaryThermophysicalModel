...
# # My Modules
# import sys
# sys.path.insert(0, '/Users/kyso3185/Documents/3D_Model/Raytracing')
#import Reflection as reflect
import Raytracing_3d as raytracing 
#import visibility
import kdtree_3d_Kya as kdtree
import planets
import orbits 

# Read in Shape Model
#import shapeModel as shape
import shapeModelMultiples as shapeModule
...
def AdvanceBinaryOrbit(priModel: shapeModule.shapeModel, secModel: shapeModule.shapeModel, priTheta,secPhi):
...
# def clipShadows(priModel: shapeModule.shapeModel, secModel: shapeModule.shapeModel, solarLocation: np.array, maxBinaryRadius = None):
...
def traceShadowsMP(iVal, priOrientation, steps, secSteps, priModel: shapeModule.shapeModel, secModel: shapeModule.shapeModel):#,secondaryFluxes):
...
def traceShadowsHighResEclipseMP(priOrientation, secOrientations, secOrientationDiff, priModel: shapeModule.shapeModel, secModel: shapeModule.shapeModel):#,secondaryFluxes):
    #print ("iVal: {}".format(iVal))
...
def binarySingleOrientationShadows(priModel: shapeModule.shapeModel, secModel: shapeModule.shapeModel, priTheta, secPhi):
...
def CalcShadowsSingleBody2(steps, stepSize,shape):
    # Attempt at vectorizing more of this 
    # Sun
    # Solar location is different for full bodies. Need to place in xy plane
    # For local, right now solar position in on the z axis 
    sLoc = np.asarray([2.124e11,0,0])
    ...
    for k in range(steps):
        ...
        dotproducts = np.einsum('ij,ij->i',directions,shape.trimesh_normals)
        incidenceAngles = np.arccos(dotproducts)
        for j in range(np.shape(dotproducts)[0]):
            if incidenceAngles[j] <= (np.pi / 2.0) or incidenceAngles[j] >= ((3*np.pi) / 2.0):
                print (np.degrees(incidenceAngles[j]))
            else: 
                dotproducts[j] = 0.
        #incidenceAngles[incidenceAngles >= (np.pi / 2.0) or incidenceAngles <= ((3*np.pi) / 2.0)] = 0
        #incidenceAngles[np.where(np.logical_and(incidenceAngles >= (np.pi / 2.0),incidenceAngles <= ((3*np.pi) / 2.0)))] = 0
        #incidenceAngles[incidenceAngles <= ((3*np.pi) / 2.0)] = 0
        print (np.count_nonzero(incidenceAngles))
